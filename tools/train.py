...
def do_train_epoch(net, data_iter, optimizer, rank, epoch, train_config):
    ...
    for (images, gt_boxes, denisty,img_masks,all_num, im_info), step in zip(data_iter, range(0, train_config.iter_per_epoch)):
        if images is None:
            continue
        # warm up
        ...
        optimizer.zero_grad()
        # forwad
        outputs = net(images.cuda(rank), img_masks.cuda(rank),im_info.cuda(rank),denisty.cuda(rank),all_num.cuda(rank), gt_boxes.cuda(rank))
        # collect the loss
        total_loss = sum([outputs[key].mean() for key in outputs.keys()])
        writer.add_scalar('loss_rpn_cls/Loss', outputs['loss_rpn_cls'], step)
        writer.add_scalar('loss_rpn_loc/Loss', outputs['loss_rpn_loc'], step)
        writer.add_scalar('loss_density/Loss', outputs['loss_density'], step)
...
def train_worker(rank, train_config, network, config):
    ...
    net.resnet50.load_state_dict(backbone_dict['state_dict'],strict=False)
    net.cuda(rank)
    begin_epoch = 1
    # build optimizer
