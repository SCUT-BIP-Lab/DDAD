...
class Density(nn.Module):
    ...
    def forward(self, featrue, all_num=0, target=None, mask=None):
        ...
        #density = F.relu(density)
        if mask is not None:
            # print("-=network-=featrue---aahudhiuw", featrue.size())
            # print("-=network-=density---aahudhiuw", density.size())
            # print("-=network-=target---aahudhiuw", target.size())
            # print("-=network-=mask---aahudhiuw", mask.size())
            ...
            # print("-=-=---aahudhiuw", density.size())
            # print("-=-=---aahudhiuw", target.size())
            loss = self.criterion(density, target)
        loss_dict = {}
        # loss_dict['loss_density'] = loss*torch.nn.Parameter(torch.ones(1)*100)
        loss_dict['loss_density'] = loss*0.1  #sssss-2 ssssss->
        loss_dict['loss_num'] = loss_num * 0.0001
        # print("network------loss_density-----",loss_dict['loss_density'])
        return loss_dict


        #return self.criterion(density, target)
