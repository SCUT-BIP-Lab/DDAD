...
class FPN(nn.Module):
    ...
    def __init__(self, bottom_up, layers_begin, layers_end, return_base_f= False):
        ...
        #self.fpn_concat_back_0 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
        BN_MOMENTUM = 0.9
        ...
        #self.denisty_2_fpn_2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
        #self.denisty_2_fpn_3 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
        #self.denisty_2_fpn_4 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        if self.return_base_f :
            # print("len(fpn)---------:", len(results))
            # print("len(density_results)---------:", len(density_results))
            # print("fpn[0]---------:",results[-0].size())
            # print("density_results[0]---------:",density_results[-0].size())
            # print("fpn[-2].size()-------------------:",results[-2].size())
            # print("density_results[-3]---------:", density_results[-3].size())
            # print("density_results[-2].size()-------------------:", density_results[-2].size())



            #tem_result = torch.cat([density_results[-1], results[-1]], axis=1)
            #print("fpn---------3:", tem_result.size())
            ...
            #results[-5] = F.relu(self.fpn_concat_back_0(torch.cat([density_results[-5], results[-5]], axis=1)))
