...
def load_gt(dict_input, key_name, key_box, class_names,creat_mask):
    assert key_name in dict_input
    if len(dict_input[key_name]) < 1:
        return np.empty([0, 5])
    else:
        assert key_box in dict_input[key_name][0]   #if no gt in record, 0 5   else: 返回gtboxes
    bbox = []
    for rb in dict_input[key_name]:
        if rb['tag'] in class_names:
            ...
            if rb["tag"] == "mask":
                tem_area = rb["fbox"]
                #print("misc_utils-=-=-=-=-=-----------")
                creat_mask[tem_area[1]:tem_area[1]+tem_area[3],tem_area[0]:tem_area[0]+tem_area[2]]=1
            tag = -1
        if 'extra' in rb:
