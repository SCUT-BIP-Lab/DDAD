...
def find_top_rpn_proposals(is_train, rpn_bbox_offsets_list, rpn_cls_prob_list,
    ...
    #train:   prev_nms_top_n=12000,   post_nms_top_n=2000
    #test:   prev_nms_top_n=6000,   post_nms_top_n=1000
    ...
    for bid in range(batch_per_gpu):
        ...
        for l in range(list_size):
            ...
            offsets = rpn_bbox_offsets_list[l][bid] \
                .permute(1, 2, 0).reshape(-1, 4)    #把channel放在最后，每四个channel对应一个offsets
            ...
            # print("123123123123",all_anchors.size())
            # print("123123123123",offsets.size())
            proposals = bbox_transform_inv_opr(all_anchors, offsets)
            if config.anchor_within_border:  #此处设计bbox是否可以超出原图片
                proposals = clip_boxes_opr(proposals, im_info[bid, :])
            probs = rpn_cls_prob_list[l][bid] \
                    .permute(1,2,0).reshape(-1, 2)
            probs = torch.softmax(probs, dim=-1)[:, 1]   #按行取softmax，之后选择第一列作为目标的分数
