...
class AnchorGenerator():
    ...
    def get_plane_anchors(self, anchor_scales: np.ndarray):
        ...
        #size是面积，ratio是等于h/w，所以S/(h/w) = w^2
        ...
        anchors = np.hstack((x_ctr - 0.5 * (ws - 1),
                         y_ctr - 0.5 * (hs - 1),
                         x_ctr + 0.5 * (ws - 1),   #减一是为了达到左上和右下，实际画一个图就很好理解了
                         y_ctr + 0.5 * (hs - 1))) - off
    ...
    def get_anchors_by_feature(self, fm_map, base_stride, off_stride):
        ...
        # fm_map.device = fm_map.device
        plane_anchors = torch.tensor(plane_anchors,device=fm_map.device)#          , device=fm_map.device)
...
if __name__ == '__main__':
    feature_map = []
    img1_path = "/home/data/CrowdHuman/Images/273271,25d0003419579b.jpg"
    img1 = cv2.imread(img1_path)
    # feature_map=np.array(img1)
    feature_map.append(np.array(img1))
    feature_map.append(np.array(img1))
    feature_map.append(np.array(img1))
    feature_map.append(np.array(img1))
    feature_map.append(np.array(img1))

    # feature_map.append(img1)
    # feature_map.append(img1)
    # feature_map.append(img1)
    # feature_map.append(img1)
    print(img1)
    c = AnchorGenerator()
    c.get_anchors_by_feature(fm_map = np.array(feature_map),base_stride = 4,off_stride =16)
    print(c)