...
class RPN(nn.Module):
    def __init__(self, rpn_channel = 256):
        ...
        #所有anchor数量乘以2，表示的是，每一个anchor，在每一个点处属于前景还是背景，
        # 相当于有2个channel的feature全是同一个anchor在每一个点的score
    ...
    def forward(self, features, im_info, boxes=None):
        ...
        for x in features:
            ...
            #这边的卷积层如何体现这些信息呢，分类和bbox
            pred_cls_score_list.append(self.rpn_cls_score(t))
            #bbox的四个channel的feature map对应一个anchor的偏移量，
            # 但数值是怎样的，怎么确定到四个点，以及四个点怎么分配
        ...
        for fm in features:
            # print("error:",fm.size(),base_stride,off_stride)
        ...
        #以上输出的rpn_rois 包括了一个torch.ones(),还有一个batch_proposals
        rpn_rois = rpn_rois.type_as(features[0])
        if self.training:
    #boxed 是GT，为什么这部分还是用rpn的结果，没有用到nms以及score重排的结果计算loss
