...
def bbox_transform_inv_opr(bbox, deltas):   #anchor ，offsets
    ...
    dw = deltas[:, 2]
    dh = deltas[:, 3]
    dw = torch.clamp(dw, max=max_delta)  #设计dw不超过max_delta
    dh = torch.clamp(dh, max=max_delta)
    pred_width = bbox_width * torch.exp(dw)   #卷积出来的deltas，通过指数作用在anchor
...
def box_overlap_ignore_opr(box, gt, ignore_label=-1):
    ...
    width_height = torch.min(box[:, None, 2:], gt[:, 2:4]) - torch.max(
        box[:, None, :2], gt[:, :2])  # [N,M,2]   #其中，N表示box中的行数，M表示gt中的行数
    #print("-=-=-=-=-=-=-width_height_shape-=-=-=-=-", width_height.shape)
    width_height.clamp_(min=0)  # [N,M,2]
    inter = width_height.prod(dim=2)  # [N,M]
    del width_height
    # handle empty boxes
    ...
    ioa = torch.where(
        inter > 0,
        inter / (area_box[:, None]),   #这里为什么要除以area？
        torch.zeros(1, dtype=inter.dtype, device=inter.device))
    #print("-=-=-=-IoA-=-=-=-=-", ioa)
